...
- `teaser-final.png`: 0921 티저의 실타래 3곳만 교체한 기준본 사본.
...
사용자가 지정한 자산입니다. 원본 PNG는 별도 0921 폴더의 `assets/icons/`에 보존했습니다. 원본 상세 페이지의 작가·라이선스 표기를 이 환경에서 확인하지 못했으므로, 외부 공개·상업 사용 전 각 원본의 이용 조건과 작가 표시 요구사항을 확인해 주세요. 이 문서는 별도의 이용권을 부여하지 않습니다.
