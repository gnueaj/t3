...
최신 수정에서는 ‘나’ 프로필을 엄마와 구분되게 바꾸고, 사진 속 두 UI 실타래를 조금 작고 또렷하게 다듬었습니다. 오른쪽 책 문구가 보이지 않도록 사진을 더 잘랐고, 얼굴이 더 보이도록 왼쪽 그라데이션을 옅게 조정했습니다. 첫 장의 `ON-DEVICE AI · CONCEPT PROPOSAL` 및 전 페이지의 반복 푸터 `WEAVE ASSIST / GALAXY AI CONCEPT`는 삭제했습니다. 페이지 번호는 유지했습니다.

최신 티저 PNG는 `assets/teaser-final.png`입니다. 제목과 왼쪽 하단 문구에 실제 Noto Sans KR·Inter를 적용했으며, `assets/teaser-typography.html`에서 해당 문구의 원본 배치를 볼 수 있습니다.
