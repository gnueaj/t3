...
## 최신 수정 · 배경 잔상 복원 / 균일한 음영 / 표지 여백

오른쪽 위 글자를 지웠던 배경에 글자 형태의 명암 흔적과 가로 경계가 남은 것을 확인했습니다. `0920/teaser-final.png`의 자연스러운 배경을 참고해 내장 imagegen으로 배경을 복원하고, 개별 글자 모양 마스크가 아닌 연속된 배경 영역으로 부드럽게 합성했습니다. 현재의 Noto Sans KR·Inter 글자 레이어는 위치·크기·색을 그대로 유지했습니다.

왼쪽 아래도 글자 형태로 남은 불규칙한 음영을 걷어낸 뒤, 296×296px·불투명도 12%의 정사각 음영을 추가했습니다. 모서리는 24px 구간에서 부드럽게 사라집니다. `teaser-typography.html`에서 음영과 글자를 각각 조정할 수 있습니다.

사진 표지 크롭은 1420×1010에서 **1463×1010**으로 바꿔 오른쪽을 43px(약 3%) 더 보여 줍니다. 사진의 시작 위치와 그라데이션 구간은 유지하고, 시작 부분의 불투명도만 조금 높여 점진적으로 밝아지게 했습니다. 어두운 구간을 제목 뒤쪽으로 늘린 것은 아닙니다.

최신 프롬프트: `teaser-background-fix.prompt.txt` · 최신 픽셀 검증: `teaser-background-validation.json`. 이번 수정 영역 두 곳 밖 변경 픽셀은 **0개**입니다. 현재 UI·아이콘·프로필·카드는 보존했습니다. 수정 전 파일은 `0920/before-0922-background-fix/`에 있습니다. 내장 imagegen을 사용했고 CLI/API fallback은 사용하지 않았습니다.

## 이전 수정 · 프로필 / 아이콘 크기 / 글꼴 / 표지 구도
...
최초 교체에서 세 아이콘 영역 밖 변경 픽셀은 0개였습니다. `teaser-icon-validation.json`과 `teaser-refinement-validation.json`은 **이전 교체 시점의 이력**이며 현재 파일의 최신 검증은 `teaser-background-validation.json`을 확인하세요. 실험용 배경을 처음 생성한 작업에는 원본 대비 픽셀 동일성을 주장하지 않습니다.
